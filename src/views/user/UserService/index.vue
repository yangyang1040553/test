    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['user:UserService:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['user:UserService:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
          v-hasPermi="['user:UserService:export']">导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" @sort-change="sortChange" :data="UserServiceList"
      @selection-change="handleSelectionChange" height="600">
...
<script>
import { listUserService, getUserService, delUserService, addUserService, updateUserService } from "@/api/user/UserService";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listUserService(this.queryParams).then(response => {
        this.UserServiceList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateUserService(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addUserService(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('user/UserService/export', {
        ...this.queryParams
      }, `UserService_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
