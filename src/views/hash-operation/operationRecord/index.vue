  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="活动类型" prop="aType">
        <el-select v-model="queryParams.aType" placeholder="请选择活动类型" clearable>
          <el-option
      ...
      <el-form-item label="是否发放" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择是否发放" clearable>
          <el-option
            v-for="dict in dict.type.is_send"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['hash-operation:operationRecord:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="是否发放" prop="status">
          <el-select v-model="form.status" placeholder="请选择是否发放">
            <el-option
              v-for="dict in dict.type.is_send"
              :key="dict.value"
              :label="dict.label"
...
export default {
  ...
  dicts: ['wallet_type', 'is_send', 'operation_type'],
  components: {
    UserInfoDialog
  },
  data() {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOperationRecord(this.queryParams).then(response => {
        this.operationRecordList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加运营活动奖励记录";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getOperationRecord(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改运营活动奖励记录";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOperationRecord(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOperationRecord(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除运营活动奖励记录编号为"' + ids + '"的数据项？').then(function () {
        return delOperationRecord(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('hash-operation/operationRecord/export', {
        ...this.queryParams
      }, `operationRecord_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
