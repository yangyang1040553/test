    ...
    <el-row :gutter="10" class="mb8">
      <!-- <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['hash-statistical:statisticalUserDay:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['hash-statistical:statisticalUserDay:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['hash-statistical:statisticalUserDay:remove']"
        >删除</el-button>
      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['hash-statistical:statisticalUserDay:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="1.5">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listStatisticalUserDay(this.queryParams).then(response => {
        this.statisticalUserDayList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateStatisticalUserDay(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addStatisticalUserDay(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除游戏玩家日统计编号为"' + ids + '"的数据项？').then(function () {
        return delStatisticalUserDay(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('hash-statistical/statisticalUserDay/export', {
        ...this.queryParams
      }, `statisticalUserDay_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
