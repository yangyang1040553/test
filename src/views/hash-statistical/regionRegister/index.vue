...
<script>
import { listRegister, getRegister, delRegister, addRegister, updateRegister } from "@/api/hash-statistical/regionRegister";
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.region)
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加用户区域注册次数";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const region = row.region || this.ids
      getRegister(region).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改用户区域注册次数";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const regions = row.region || this.ids;
      this.$modal.confirm('是否确认删除用户区域注册次数编号为"' + regions + '"的数据项？').then(function () {
        return delRegister(regions);
      }).then(() => {
        this.getList();
